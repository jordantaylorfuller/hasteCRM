...
describe("PipelineBoard", () => {
  ...
  it("renders pipeline board with stages", () => {
    render(<PipelineBoard pipeline={mockPipeline} deals={mockDeals} />);
  ...
  it("handles empty deals array", () => {
    render(<PipelineBoard pipeline={mockPipeline} deals={[]} />);
  ...
  it("handles stage with no deals in dealsByStage map", () => {
    ...
    jest.spyOn(React, "useMemo").mockImplementation((fn, deps) => {
      ...
      if (callCount === 1) {
        return originalUseMemo(() => {
          ...
          grouped.set(
            "stage-2",
            mockDeals.filter((d) => d.stage.id === "stage-2"),
          );
          grouped.set(
            "stage-3",
            mockDeals.filter((d) => d.stage.id === "stage-3"),
          );
    ...
    render(
      <PipelineBoard pipeline={pipelineWithExtraStage} deals={mockDeals} />,
    );
    ...
    expect(stage4).toHaveTextContent("USD 0"); // No deals, so value is 0
