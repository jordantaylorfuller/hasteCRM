...
jest.mock("lucide-react", () => ({
  ...
  Circle: ({ className }: { className?: string }) => (
    <div data-testid="circle-icon" className={className}>Circle</div>
  ),
}));
...
describe("DropdownMenu", () => {
  ...
  describe("Basic functionality", () => {
    ...
    it("respects disabled items", async () => {
      ...
      fireEvent.click(screen.getByText("Open Menu"));
      await waitFor(() => expect(screen.getByText("Disabled Item")).toBeInTheDocument());
  ...
  describe("DropdownMenuShortcut", () => {
    it("renders shortcut text", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText("⌘S")).toHaveClass("ml-auto", "text-xs", "opacity-60");
      });
    });
