  ...
  return (
    <div {...props} data-testid="select-root">
      {React.Children.map(children, child =>
        React.isValidElement(child) ? React.cloneElement(child, { ...contextValue } as any) : child
      )}
    </div>
  );
};
...
export const Trigger = React.forwardRef(({ children, className, value, ...props }: any, ref: any) => (
  <button ref={ref} className={className} role="combobox" aria-expanded="false" {...props}>
    {React.Children.map(children, child =>
      React.isValidElement(child) && child.type === Value ? React.cloneElement(child, { value } as any) : child
    )}
  </button>
));
...
export const Portal = ({ children, ...props }: any) => <div {...props}>{children}</div>;
export const Content = React.forwardRef(({ children, className, position = "popper", value, onValueChange, ...props }: any, ref: any) => {
  // Add the expected classes
  const combinedClassName = `relative z-50 bg-popover ${className || ''}`.trim();
  return (
    <div ref={ref} className={combinedClassName} data-position={position} {...props}>
      {React.Children.map(children, child =>
...
export const Item = React.forwardRef(({ children, className, value, onValueChange, ...props }: any, ref: any) => {
  const handleClick = () => {
  ...
  return (
    <div ref={ref} className={className} role="option" onClick={handleClick} {...props}>
      {children}
    </div>
  );
});
...
export const ItemText = ({ children, ...props }: any) => <span {...props}>{children}</span>;
...
export const Viewport = ({ children, className, ...props }: any) => (
  <div className={className} role="listbox" {...props}>{children}</div>
);