...
export function DayPicker({
  ...
  const currentDate = selected instanceof Date ? selected : new Date(2024, 0, 15);
  ...
  const handleDateClick = (date: Date) => {
    if (disabled && ((typeof disabled === 'function' && disabled(date)) || disabled === true)) {
      return;
    }
  ...
  const isSelected = (date: Date) => {
    ...
    if (mode === 'range' && typeof selected === 'object' && 'from' in selected) {
      ...
      if (range.from && format(date, 'yyyy-MM-dd') === format(range.from, 'yyyy-MM-dd')) return true;
      if (range.to && format(date, 'yyyy-MM-dd') === format(range.to, 'yyyy-MM-dd')) return true;
    }
  ...
  return (
    <div className={`rdp-root ${className || ''}`} data-mode={mode} role="application">
                ...
                {Array.from({ length: Math.ceil(days.length / 7) }, (_, weekIndex) => (
                  <tr key={weekIndex} className={classNames?.row || ''}>
                    {days.slice(weekIndex * 7, (weekIndex + 1) * 7).map((day, dayIndex) => {
                      ...
                      if (selected) dayClassName += ' ' + (classNames?.day_selected || '');
                      if (today) dayClassName += ' ' + (classNames?.day_today || '');
                      if (outside) {
                        if (showOutsideDays) {
                          dayClassName += ' ' + (classNames?.day_outside || '') + ' opacity-50';
                        } else {
                          dayClassName += ' ' + (classNames?.day_hidden || '') + ' invisible';
                        }
                      }
                      if (disabled) dayClassName += ' ' + (classNames?.day_disabled || '');
