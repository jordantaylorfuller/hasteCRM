...
test.describe("Pipeline Management", () => {
  ...
  test("should display pipelines page", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /pipeline.*settings/i }),
    ).toBeVisible();
  });
  ...
  test("should open add deal modal", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /add.*deal/i }),
    ).toBeVisible();
  ...
  test("should handle drag and drop", async ({ page }) => {
    ...
    if (await dealCard.isVisible()) {
      // Get initial stage
      const initialStage = await dealCard
        .locator("..")
  ...
  test("should open deal details", async ({ page }) => {
    ...
    if (await dealCard.isVisible()) {
      ...
      await expect(
        page
          .getByRole("heading", { name: /deal.*detail/i })
  ...
  test("should handle pipeline settings", async ({ page }) => {
    ...
    await expect(page.getByRole("dialog")).toBeVisible();
    await expect(
      page.getByRole("heading", { name: /pipeline.*settings/i }),
    ).toBeVisible();
