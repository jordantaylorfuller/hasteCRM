...
test.describe("Contacts Management", () => {
  ...
  test("should search contacts", async ({ page }) => {
    ...
    await expect(
      page.getByText(/searching/i).or(page.getByText(/no results/i)),
    ).toBeVisible();
  });
  ...
  test("should apply filters", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /filter/i }).getByText("1"),
    ).toBeVisible();
  });
  ...
  test("should handle contact click", async ({ page }) => {
    ...
    if (await contactCard.isVisible()) {
      ...
      await expect(
        page
          .getByRole("heading", { name: /contact.*detail/i })
  ...
  test("should handle bulk actions", async ({ page }) => {
    ...
    if ((await checkboxes.count()) > 1) {
      ...
      await expect(
        page.getByRole("button", { name: /bulk.*action/i }),
      ).toBeVisible();
    }
  });
});
