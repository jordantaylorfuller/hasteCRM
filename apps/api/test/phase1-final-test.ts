...
async function runFinalTests() {
  ...
  try {
    ...
    const meResponse = await axios.post(`${API_URL}/auth/me`, {}, {
    ...
    if (meResponse.data.user) {
      ...
      console.log(`   Workspace: ${meResponse.data.workspace?.name || 'No workspace'}`);
    }
    ...
    console.log(`   Backup codes: ${setupResponse.data.backupCodes.length} generated`);
    ...
    if (login2FAResponse.data.requiresTwoFactor) {
      ...
      const login2FAVerifyResponse = await axios.post(`${API_URL}/auth/login/2fa`, {
        email: testEmail,
        password: testPassword,
        token: newTotpToken,
      });
    ...
    console.log(`✅ Sessions retrieved: ${sessionsResponse.data.length} active sessions`);
