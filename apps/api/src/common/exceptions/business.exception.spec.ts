...
describe("BusinessException", () => {
  ...
  describe("BusinessExceptions factory methods", () => {
    ...
    describe("duplicateResource", () => {
      it("should create duplicate resource exception", () => {
        const exception = BusinessExceptions.duplicateResource(
          "Contact",
          "email",
        );
      ...
      it("should handle duplicate without field", () => {
        ...
        const response = exception.getResponse() as any;
        expect(response.userMessage).toBe(
          "A workspace with this information already exists.",
        );
      });
    });
    ...
    describe("rateLimitExceeded", () => {
      it("should create rate limit exceeded exception", () => {
        const exception = BusinessExceptions.rateLimitExceeded(
          100,
          "5 minutes",
        );
        ...
        expect(response.userMessage).toBe(
          "You've made too many requests. Please wait 5 minutes before trying again.",
        );
  ...
  describe("BusinessErrorCode enum", () => {
    it("should have all expected error codes", () => {
      ...
      expect(BusinessErrorCode.AI_SERVICE_UNAVAILABLE).toBe(
