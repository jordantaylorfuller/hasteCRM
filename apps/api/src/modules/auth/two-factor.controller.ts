...
export class TwoFactorController {
  ...
  async disableTwoFactor(
    ...
    await this.twoFactorService.disableTwoFactor(req.user.sub, password, token || '');
  ...
  async recoverWithBackupCode(
    ...
    const isValid = await this.twoFactorService.verifyBackupCode(req.user.sub, backupCode);
    if (!isValid) {
