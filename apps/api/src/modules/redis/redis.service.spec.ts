...
describe('RedisService', () => {
  ...
  beforeEach(async () => {
    ...
    (Redis as jest.MockedClass<typeof Redis>).mockImplementation((config: any) => {
  ...
  describe('session management', () => {
    ...
    describe('getSession', () => {
      it('should retrieve session data', async () => {
        ...
        expect(mockSessionClient.get).toHaveBeenCalledWith('session:session-123');
      });
    ...
    describe('deleteSession', () => {
      it('should delete session', async () => {
        ...
        expect(mockSessionClient.del).toHaveBeenCalledWith('session:session-123');
      });
    });
    ...
    describe('extendSession', () => {
      it('should extend session TTL', async () => {
        ...
        expect(mockSessionClient.expire).toHaveBeenCalledWith('session:session-123', 7200);
      });
  ...
  describe('token blacklist management', () => {
    describe('blacklistToken', () => {
      it('should blacklist a token', async () => {
        ...
        expect(mockRedisClient.setex).toHaveBeenCalledWith('blacklist:token-123', 3600, '1');
      });
    });
  ...
  describe('user session tracking', () => {
    describe('getUserSessions', () => {
      it('should get all sessions for a user', async () => {
        mockSessionClient.keys.mockResolvedValue(['session:123', 'session:456', 'session:789']);
      ...
      it('should handle invalid session data', async () => {
        mockSessionClient.keys.mockResolvedValue(['session:123', 'session:456']);
        mockSessionClient.get
    ...
    describe('invalidateUserSessions', () => {
      it('should delete all user sessions', async () => {
        jest.spyOn(service, 'getUserSessions').mockResolvedValue(['123', '456']);
  ...
  describe('proxy methods', () => {
    ...
    describe('hincrby', () => {
      it('should increment hash field', async () => {
        ...
        expect(result).toBe(10);
        expect(mockRedisClient.hincrby).toHaveBeenCalledWith('hash-key', 'field', 5);
      });
    });
