...
describe('GmailHistoryService', () => {
  ...
  describe('syncHistory', () => {
    ...
    it('should record sync error on failure', async () => {
      ...
      await expect(service.syncHistory('account-123')).rejects.toThrow('API Error');
  ...
  describe('processHistory', () => {
    it('should process messages added', async () => {
      ...
      const result = await (service as any).processHistory(mockAccount, changes);
  ...
  describe('queue operations', () => {
    it('should queue message fetch', async () => {
      await (service as any).queueMessageFetch('account-123', 'msg-123', 'thread-123');
