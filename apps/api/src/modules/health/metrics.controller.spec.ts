...
describe('MetricsController', () => {
  ...
  describe('getPrometheusMetrics', () => {
    it('should return metrics in Prometheus format', async () => {
      ...
      expect(result).toContain('# HELP app_uptime_seconds Application uptime in seconds');
      expect(result).toContain('# TYPE app_uptime_seconds gauge');
      expect(result).toMatch(/app_uptime_seconds \d+/);
      
      expect(result).toContain('# HELP app_memory_heap_used_bytes Heap memory used in bytes');
      expect(result).toContain('# TYPE app_memory_heap_used_bytes gauge');
      
      expect(result).toContain('# HELP app_http_requests_total Total number of HTTP requests');
  ...
  describe('getDatabaseMetrics', () => {
    it('should query active database connections', async () => {
      ...
      expect(mockPrismaService.$queryRaw).toHaveBeenCalledWith(
        expect.arrayContaining([expect.stringContaining('SELECT COUNT(*) as count')])
      );
    ...
    it('should handle BigInt conversion', async () => {
      mockPrismaService.$queryRaw.mockResolvedValue([{ count: BigInt(9007199254740992) }]);
